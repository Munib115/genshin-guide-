    ...
    <div className="chat-layout" style={{ display: "grid", gridTemplateColumns: "1fr", gap: "var(--spacing-lg)", marginTop: "var(--spacing-lg)" }}>
      ...
      <div 
        style={{ 
          ...
          borderLeft: `4px solid ${connectionStatus === "CONNECTED" ? "var(--primary)" : connectionStatus === "CONNECTING" ? "#ffb300" : "#d32f2f"}`,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span 
            style={{ 
              ...
              height: "10px", 
              borderRadius: "50%", 
              backgroundColor: connectionStatus === "CONNECTED" ? "var(--primary)" : connectionStatus === "CONNECTING" ? "#ffb300" : "#d32f2f",
              display: "inline-block"
            }} 
          />
          <span className="caption-xs" style={{ color: "var(--on-dark)" }}>
            SUPABASE LIVE SYNC: <strong style={{ color: connectionStatus === "CONNECTED" ? "var(--primary)" : "inherit" }}>{connectionStatus}</strong>
          </span>
        </div>
        <button 
          className="btn btn-outline-on-dark" 
          style={{ padding: "4px 10px", fontSize: "12px", border: "1px solid var(--hairline-strong)", cursor: "pointer" }}
          onClick={() => setShowConfig(!showConfig)}
        >
          {showConfig ? "Hide Settings" : "Configure Supabase"}
        </button>
      </div>

      {/* Supabase Configuration Drawer/Section */}
      {showConfig && (
        <div className="card-dark" style={{ border: "1px solid var(--primary)", background: "rgba(0,0,0,0.85)" }}>
          ...
          <form onSubmit={handleSaveConfig} style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            <div>
              <label className="caption-xs" style={{ display: "block", color: "var(--on-dark)", marginBottom: "4px" }}>SUPABASE URL</label>
              <input
                type="url"
                required
                className="device-select"
                style={{ width: "100%", padding: "10px", background: "#222", border: "1px solid var(--hairline-strong)", color: "white" }}
                value={supabaseUrl}
            ...
            <div>
              <label className="caption-xs" style={{ display: "block", color: "var(--on-dark)", marginBottom: "4px" }}>SUPABASE ANON KEY</label>
              <input
                type="password"
                required
                className="device-select"
            ...
            <div style={{ display: "flex", gap: "10px", marginTop: "8px" }}>
              <button type="submit" className="btn btn-primary" style={{ padding: "8px 16px" }}>Save & Connect</button>
              <button type="button" className="btn btn-outline-on-dark" onClick={handleClearConfig} style={{ padding: "8px 16px" }}>Clear Saved Credentials</button>
            </div>
          </form>
        </div>
      )}
