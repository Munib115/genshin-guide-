...
// Enable Cross-Origin Resource Sharing (CORS) so local frontend can query the API
app.use(cors());
...
const pool = new Pool({
  ...
  database: process.env.PGDATABASE || "teyvatresonance",
  password: process.env.PGPASSWORD || "postgres",
  port: parseInt(process.env.PGPORT || "5432"),
});

// Seed data to initialize the database if empty
...
// Database Initialization (Schema creation + seeding if empty)
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS characters (
        ...
        icon TEXT,
        splash TEXT,
        best_weapon JSONB NOT NULL,
        f2p_weapon JSONB NOT NULL,
        best_artifacts JSONB,
        best_echoes JSONB,
        team_comps JSONB NOT NULL,
        pakistani_tips TEXT
      );
    `);
    console.log("Database table structure checked/created successfully.");
    ...
    if (count === 0) {
      console.log("Database is empty. Seeding character guide entries...");
      
      for (const char of seedCharacters) {
        const queryText = `
          INSERT INTO characters (
            id, name, game, rarity, element, weapon, role, description, icon, splash, 
            best_weapon, f2p_weapon, best_artifacts, best_echoes, team_comps, pakistani_tips
        ...
        const values = [
          ...
          char.icon,
          char.splash,
          JSON.stringify(char.bestWeapon),
          JSON.stringify(char.f2pWeapon),
          char.bestArtifacts ? JSON.stringify(char.bestArtifacts) : null,
          char.bestEchoes ? JSON.stringify(char.bestEchoes) : null,
          JSON.stringify(char.teamComps),
          char.pakistaniTips
        ];

        await client.query(queryText, values);
      }
      console.log(`Successfully seeded ${seedCharacters.length} character entries!`);
    } else {
      console.log(`Database already initialized with ${count} entries.`);
    }

  } catch (err) {
    console.error("Error initializing database schema or seeding data:", err.message);
    console.log("Make sure the database 'teyvatresonance' exists. You may need to create it manually using 'CREATE DATABASE teyvatresonance;'");
  } finally {
...
// Helper function to map SQL row structure to client JSON camelCase schema
function mapRowToClient(row) {
  return {
    ...
    icon: row.icon,
    splash: row.splash,
    bestWeapon: row.best_weapon,
    f2pWeapon: row.f2p_weapon,
    bestArtifacts: row.best_artifacts,
    bestEchoes: row.best_echoes,
    teamComps: row.team_comps,
    pakistaniTips: row.pakistani_tips
  };
}

// 1. Get all character guides
app.get("/api/characters", async (req, res) => {
  try {
    const { rows } = await pool.query("SELECT * FROM characters ORDER BY game, name");
    const mapped = rows.map(mapRowToClient);
    res.json(mapped);
  } catch (err) {
    console.error("Error executing query:", err.message);
    res.status(500).json({ error: "Failed to fetch character entries from database", details: err.message });
  }
});

// 2. Get single character guide details
app.get("/api/characters/:id", async (req, res) => {
  try {
    const { rows } = await pool.query("SELECT * FROM characters WHERE id = $1", [req.params.id]);
    
    if (rows.length === 0) {
      return res.status(404).json({ error: `Character guide ID '${req.params.id}' not found.` });
    }
    
    res.json(mapRowToClient(rows[0]));
  } catch (err) {
    console.error("Error executing query:", err.message);
    res.status(500).json({ error: "Database error during character query", details: err.message });
  }
});
...
app.listen(PORT, () => {
  console.log(`TeyvatResonance PK backend running on http://localhost:${PORT}`);
});
